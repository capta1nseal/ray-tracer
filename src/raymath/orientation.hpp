...
*/
template <Vec3Basis T> struct Orientation {
    T roll, pitch, yaw;
    ...
    Orientation(const Orientation<U> &other)
    ...
    Vec3<T> forward() const {
        ...
        // x here temporarily stands for length of the direction vector
    ...
    Vec3<T> up() const {
        ...
        return Vec3(minusCosRollSinPitch * cosYaw + sinRoll * sinYaw,
    ...
    template <Vec3Basis U> auto operator+(const Orientation<U> &other) const {
        return Orientation<std::common_type_t<T, U>>(
            roll + other.roll, pitch + other.pitch, yaw + other.yaw);
    }
...
std::ostream &operator<<(std::ostream &os, const Orientation<T> &orientation) {
    os << "(" << orientation.roll << "," << orientation.pitch << ","
       << orientation.yaw << ")";
    return os;
}
