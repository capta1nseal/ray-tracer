...
// Uses fast bitwise and logical operations to determine whether an unsigned int
// is a power of two. (has exactly one bit set)
bool isPowerOfTwo(unsigned int x);

// Uses fast bitwise and logical operations to determine whether an unsigned int
// is a power of two. This implementation skips a check for zero, so returns
// true also when x is zero.
bool isZeroOrPowerOfTwo(unsigned int x);

// Returns the sum of a single vector's elements.
template <Vec3Basis T> auto sumElements(const Vec3<T> &vector) {
...
// Doesn't expect anything specific of the values given, going outside the range
// [0,1] is well defined.
template <Vec3Basis T, Vec3Basis U>
auto lerp(const Vec3<T> &first, const Vec3<U> &second,
          std::common_type_t<T, U> factor) {
    return Vec3<std::common_type_t<T, U>>(first + (factor * (second - first)));
}

// Returns a vector with each of given vector's values, clamped between min and
// max.
template <Vec3Basis T> Vec3<T> clamp(const Vec3<T> &vector, T min, T max) {
    return Vec3<T>(std::clamp(vector.x, min, max),
                   std::clamp(vector.y, min, max),
                   std::clamp(vector.z, min, max));
}

// Reflects direction off normal's origin.
// Preferable should have negative dot product with direction, but still works
// inverted.
template <Vec3Basis T, Vec3Basis U>
auto reflect(const Vec3<T> &direction, const Vec3<U> &normal) {
    return Vec3<std::common_type_t<T, U>>(2.0 * (normal * -direction) * normal +
                                          direction);
}
...
template <Vec3Basis T> T greatestElementByMagnitude(const Vec3<T> &vector) {
    T xMag = std::abs(vector.x);
    T yMag = std::abs(vector.y);
    T zMag = std::abs(vector.z);
    if (xMag >= yMag and xMag >= zMag)
        return vector.x;
    if (yMag >= zMag)
        return vector.y;
    return vector.z;
...
template <Vec3Basis T> T mean(const Vec3<T> &vector) {
    return (vector.x + vector.y + vector.z) * T(oneThird);
}

// Returns mean of x,y,z's magnitudes.
template <Vec3Basis T> T meanMagnitude(const Vec3<T> &vector) {
...
template <Vec3Basis T, Vec3Basis U>
auto rotateAround(const Vec3<T> &vector, const Vec3<U> &axis,
                  std::common_type_t<T, U> angle) {
    using ResultType = std::common_type_t<T, U>;
    // Component of vector parallel to axis.
    Vec3<ResultType> parallelComponent =
        ((vector * axis) / (axis * axis)) * axis;
    ...
    // sqrt of proportion between square lengths saves a sqrt compared to two
    // magnitude calculations.
    return parallelComponent + std::cos(angle) * perpendicularComponent +
           std::sin(-angle) *
               std::sqrt((perpendicularComponent * perpendicularComponent) /
...
// Returns vector rotated around axis anticlockwise by angle.
template <Vec3Basis T, Vec3Basis U>
auto rotateAroundUnit(const Vec3<T> &vector, const Vec3<U> &axis,
                      std::common_type_t<T, U> angle) {
    using ResultType = std::common_type_t<T, U>;
    // Component of vector parallel to axis.
