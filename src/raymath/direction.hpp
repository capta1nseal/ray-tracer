...
*/
template <Vec3Basis T> struct Direction {
    T azimuth, altitude;
    ...
    Direction(const Direction<U> &other)
    ...
    template <Vec3Basis U> auto operator+(const Direction<U> other) {
        return Direction<std::common_type_t<T, U>>(azimuth + other.azimuth,
    ...
    void sanitize() {
        if (!std::isnormal(azimuth))
            azimuth = T(0.0);
        if (!std::isnormal(altitude))
            altitude = T(0.0);
    }
};
