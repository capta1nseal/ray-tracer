...
template <typename T> struct is_vec3basis : std::is_floating_point<T> {};
...
normalize and normalized methods will explicitly check for 0 and 1-length
vectors.
*/
template <Vec3Basis T> struct Vec3 {
    T x, y, z;

    Vec3(T initX = T(0.0), T initY = T(0.0), T initZ = T(0.0))
    ...
    template <Vec3Basis U> auto operator+(const Vec3<U> &other) const {
        return Vec3<std::common_type_t<T, U>>(x + other.x, y + other.y,
                                              z + other.z);
    }

    template <Vec3Basis U = T> auto operator+(U scalar) const {
        return Vec3<std::common_type_t<T, U>>(x + scalar, y + scalar,
                                              z + scalar);
    }
    ...
    template <Vec3Basis U> auto operator%(const Vec3<U> &other) const {
        return Vec3<std::common_type_t<T, U>>(y * other.z - z * other.y,
                                              z * other.x - x * other.z,
                                              x * other.y - y * other.x);
    }
    ...
    // multiplicative assignment is an element-wise multiplication by other's
    // elements, not dot product.
    template <Vec3Basis U> Vec3 &operator*=(const Vec3<U> &other) {
        x *= other.x;
        y *= other.y;
        z *= other.z;
    ...
    // divisive assignment is an element-wise division by other's elements, not
    ...
    T operator()() const { return std::sqrt(x * x + y * y + z * z); }

    void normalize() {
        T length = (*this)();
        if (length == T(0.0) or length == T(1.0))
            return;
        length = T(1.0) / length;
        x *= length;
        y *= length;
...
template <Vec3Basis T, Vec3Basis U = T>
auto operator*(U scalar, const Vec3<T> &vec3) {
    return vec3 * scalar;
}
