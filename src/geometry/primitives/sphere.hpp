...
struct Sphere : public Primitive {
    ...
    HitInfo intersectRay(const Ray &ray) const override {
        ...
        double midDifferenceSquared =
            toCenter * toCenter - midDistance * midDistance;
        double radiusSquared = radius * radius;

        // Return early if ray doesn't intersect forwards or backwards.
        if (midDifferenceSquared > radiusSquared)
            return hitInfo;

        double halfDepth = std::sqrt(radiusSquared - midDifferenceSquared);

        // Select entry point if it's in front of the ray origin, otherwise exit
        // point. Tolerance of a picometer to avoid light leaking.
        hitInfo.distance = (midDistance > halfDepth + 1.0e-12)
                               ? midDistance - halfDepth
                               : midDistance + halfDepth;
        ...
        hitInfo.normal = (midDistance > halfDepth)
                             ? (hitInfo.hitPoint - center) * (1.0 / radius)
                             : (center - hitInfo.hitPoint) * (1.0 / radius);
