...
struct Plane : public Primitive {
    // Position of one rectangle corner from world origin. Acts as origin for
    // edge1 and edge2.
    Vec3<double> corner;
    ...
    Plane(const Vec3<double> &initCorner = {},
          const Vec3<double> &initEdge1 = {},
          const Vec3<double> &initEdge2 = {})
    ...
    HitInfo intersectRay(const Ray &ray) const override {
        ...
        // Temporarily represents dot product of signed area vector and ray
        // direction.
        ...
        hitInfo.distance =
            (hitInfo.normal * (corner - ray.origin)) / hitInfo.distance;

        if (hitInfo.distance <= 1.0e-12)
            return hitInfo;

        // Temporarily represents local coordinates of hitPoint.
        hitInfo.hitPoint =
            ray.origin + (ray.direction * hitInfo.distance) - corner;
        ...
        // Area of subparallelogram multiplied by area of full parallelogram
        // through dot product is much cheaper than the sqrt in a magnitude
        // call. This can be compared with the full area squared to determine
        // intersection. If it is negative, it means the areas do not overlap,
        // therefore the point is outside the parallelogram.
        if (subArea1 < 0.0 or subArea2 < 0.0 or subArea1 > areaSquared or
            subArea2 > areaSquared)
            return hitInfo;
        ...
        hitInfo.normal =
            (hitInfo.normal * ray.direction > 0.0 ? -hitInfo.normal
                                                  : hitInfo.normal);
