...
public:
    ...
    // Run whenever anything about camera changed, or the camera changes
    // completely.
    void setCamera(Camera &newCamera);

    // Set new maximum number of samples per pixel.
    // Does not affect image generated, but may stop its render immediately if
    // decreased.
    ...
    // Generate outgoing ray direction based on incoming ray, surface normal and
    // material properties.
    Vec3<double> bounceDirection(const Vec3<double> &incomingRay,
    ...
    Vec3<double>
    getRandomUniformDirectionHemisphere(const Vec3<double> &normal) const;
    // Cosine-biased hemisphere distribution.
    // This means low-angle diffuse reflections don't need scaled down, instead
    // they are just sampled less.
    Vec3<double>
    getRandomBiasedDirectionHemisphere(const Vec3<double> &normal) const;
