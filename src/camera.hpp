...
public:
    ...
    Plane getTargetPlane() const {
        ...
        Vec3<double> toCorner =
            forwardVec * toPlane + rightVec * -toLeft + upVec * -toBottom;
    ...
    // Get a ray from camera position pointing towards the center of pixel (x,
    // y) within the set resolution.

    Ray getRayToPixel(unsigned int x, unsigned int y) const {
        return getRayToSubPixel(double(x) + 0.5, double(y) + 0.5);
    }
    // Get a ray from camera position pointing towards a raydom sub-pixel
    // position within the given pixel. The pixel is defined according to the
    // constructed target plane, width and height.
    Ray getRandomRayToPixel(unsigned int x, unsigned int y) const {
        return getRayToSubPixel(double(x) + randomGenerator->randomLinearUnit(),
                                double(y) +
    ...
    Ray getRayToSubPixel(double x, double y) const {
        return {position, ((targetPlane.corner - position) +
                           targetPlane.edge1 * (x / double(width)) +
                           targetPlane.edge2 * (y / double(height)))
                              .normalized()};
    }
...
private:
    ...
    // rays will be cast towards this plane, centered exactly 1 m in front of
    // the camera. the origin of the plane will be in the bottom left from the
